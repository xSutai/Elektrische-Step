...
void setup() {
  ...
  //is called when the battery reaches 15% or 10%. beeps to let the user know it's battery is almost dead.
void beep(long time, int amount){
  ...
  //is called when the hall sensor detects a difference in value. this calculate the rps of the motor. (in the future it will calculate the speed in kph)
void detect(){
...
void loop() {
  ...
  if(!halldetect && analogRead(hallpin)>500){
    detect();
  }
  else if(halldetect && analogRead(hallpin)<100){
    detect();
  }
